...
const MensWear = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://product-gallery.onrender.com/api/menswear');
        if (!response.ok) {
        ...
        setError('Unable to fetch . Please check your internet connection');  
      } finally {
        setLoading(false);        
      }
    };
  ...
  return (
    <div>
      <h1 style={{ textAlign: 'left', paddingLeft: '100px' }} >Men's</h1>
